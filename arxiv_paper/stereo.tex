...
We introduce the \textit{M\"obius transformations}, which have several characterizations.  Here we present an algebraic one to match the derivation in \Cref{lem:stereocirc}.  To make the presentation clearer, we will work with the complex numbers $\C$ rather than $\R^2$.
